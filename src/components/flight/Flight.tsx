  ...
  return <div className={classNames('row align-items-center', className)}>
    <div className={'col-3 d-flex align-items-center'}>
      <div className={'flex-grow-1'}>
        ...
        <div className={styles.airport}>{flight.origin.name} ({flight.origin.code})</div>
      </div>
      <NextIcon/>
    </div>
    <div className={'col-3'}>
      {format(parseISO(flight.arrival_date), 'MMM dd, HH:MM')}
      <div className={styles.airport}>{flight.destination.name} ({flight.destination.code})</div>
    </div>
